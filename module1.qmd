...
List your team member(s): None other than me (Anna Figge)

Include a link to your forked GH repository: <https://github.com/AFhmc/CLES131-module1-climate>

Include a link to your .qmd file: <https://github.com/AFhmc/CLES131-module1-climate/blob/main/module1.qmd>
...
```{r}
long_term_co2 <-read_table("data/vostok_icecore_co2.txt", 
                  comment="*",
                  col_names = c("Depth", "Ice_Age", "Air_Age", "mean_CO2_conc"), skip = 21,
                  na = c("-1", "-99.99"))
head(long_term_co2)
```

The data was obtained by measurements of air trapped in ice cores at Vostok; measuring at different depths the age of the ice (calculated from depth based on prior scientific knowledge), calculating the age of the air trapped in that ice (based on the age of the ice and accumulation rate), and finding the concentration of CO2 in that ice. The columns are depth of ice, which has units of meters; the age of the ice, which has units of years before present (where present is defined as 1950, according to my googling); the mean age of the air trapped in that ice, which also has units of years before present, and the CO2 concentration of that ice, which has units of parts per million.
...
```{r}
#get year of air; 1950 - mean air age
long_term_co2<-long_term_co2 |> mutate(AirYear = 1950 - Air_Age)
#order chronologically
long_term_co2 <- long_term_co2 |> arrange(AirYear)
head(long_term_co2)
...
```{r}
#Make function to get moving average of window size s (s in air years, such that the value for each year is the average of all points with years between year-s/2 and year+s/2 ). Takes however many points are in that range (so the edge points will be a bit weird)

#Note: I googled how to define a function, and some other nitpicky type handling things

moving_window_avg <- function(df, s) {
  output <- vector(length = nrow(df))
  for (row in 1:nrow(df)) {
    row_year <- pull(df, AirYear)[row]
    window_mean <- df |> filter(between(AirYear, row_year - s/2, row_year + s/2)) |> summarise(mean(mean_CO2_conc))
    output[row] <- window_mean[[1]]
  }
  return(output)
}
...
#Use on data: 
long_term_co2 <- long_term_co2 |>
  mutate(moving_window_5000 = moving_window_avg(long_term_co2,5000),
         moving_window_50000 = moving_window_avg(long_term_co2,50000))
head(long_term_co2)
```

```{r}
#Pivot long so I can colorcode 
pivoted_long_term_co2 <- long_term_co2 |> 
  pivot_longer(cols = c(mean_CO2_conc, moving_window_5000, moving_window_50000), 
               names_to = "Smoothing Scheme", values_to = "CO2conc")



#I googled how to change legend labels
pivoted_long_term_co2 |> ggplot(aes(x=AirYear, y = CO2conc, color = `Smoothing Scheme`)) + geom_point() + geom_line() + 
  scale_color_discrete(labels = c("Unsmoothed", "5000 Year Moving Window", "50,000 Year Moving Window")) +
  labs(x = "Mean Year of Air Source (ref 0 CE)", y = "CO2 Concentration (ppm)")
```
...
I will plot the unsmoothed data to avoid confusion, since the Mauna Loa data is unsmoothed

```{r}

#Get only the air years and average CO2 conc for the longterm data
long_term_co2_only_avg <- long_term_co2 |> 
  rename(all_of(c(year = "AirYear", average= "mean_CO2_conc"))) |> select(c(year, average))

#Get column of dataset
co2 <- co2 |> mutate(dataset = "Mauna Loa")
long_term_co2_only_avg <- long_term_co2_only_avg |> mutate(dataset = "Vostok Ice Core")

#combine datasets
combined_co2 <- rbind(long_term_co2_only_avg,select(co2, c(year, average,dataset)))
```

```{r}
#plot
combined_co2 |> ggplot(aes(x = year, y =average, color = dataset)) + geom_point() + geom_line() + labs(x = "Year", y = "CO2 concentration (ppm)", color = "Data source")
```
...
```{r}
combined_co2 |> ggplot(aes(x = year, y =average, color = dataset)) + geom_point() + geom_line() + labs(x = "Year", y = "CO2 concentration (ppm)", color = "Data source", title = "Highest CO2 Concentration in 400,000 years")
```

For more than 400,000 years, CO2 concentrations followed a cycle oscillating from around 200 to 300 ppm, with each cycle lasting around 100,000 years. Now, we have increased CO2 concentrations by over 100ppm in the last century alone, resulting in CO2 values more than 100ppm higher than have been seen in more than 400,000 years.
