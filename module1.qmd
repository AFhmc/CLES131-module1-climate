...
long_term_co2 <-read_table("data/vostok_icecore_co2.txt", 
                  comment="*",
                  col_names = c("Depth", "Ice_Age", "Air_Age", "CO2_conc"), skip = 21,
                  na = c("-1", "-99.99"))
head(long_term_co2)
```
...
```{r}
#Get moving average over 5 thousand years

```

```{r}
long_term_co2 |> ggplot(aes(x=AirYear, y = CO2_conc)) + geom_point() + geom_line()
```
